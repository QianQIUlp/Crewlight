...
Codex lifecycle hooks invoke Crewlight with the event name in
`--hook <EventName>` and may send one JSON payload on stdin. Crewlight follows
...
absolute CLI entry path. Use `--binary crewlight` only when the Codex hook
...
quoted executable path. Crewlight therefore emits an unquoted
...
2. open `/hooks`;
3. inspect the source and complete Crewlight command;
4. trust it only when the executable path and arguments match your installation.

Crewlight does not bypass or weaken this review flow.

## Observed events

| Codex hook          | Crewlight status     |
| ------------------- | -------------------- |
...
When `--hook` is absent for backward compatibility, Crewlight falls back to the
...
directory, and a bounded tool name where useful. Crewlight does not read or
...
Crewlight treats Codex hooks as fire-and-forget observations. Every path exits
...
Crewlight does not return hook response fields, warnings, debug text, prompts,
tool input/output, transcripts, or complete payloads. This keeps the integration
observation-only and prevents Crewlight from blocking or modifying Codex
behavior.
...
`crewlight setup codex-hooks --print --surface desktop` and follow the
