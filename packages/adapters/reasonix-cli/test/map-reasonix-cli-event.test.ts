...
import {
  ingestReasonixCliHookJson,
  mapReasonixCliEvent,
} from "../src/index.js";

function eventFor(payload: Record<string, unknown>) {
  const result = mapReasonixCliEvent({
    session_id: "reasonix-cli-session",
    cwd: "/tmp/reasonix-cli-project",
    ...payload,
  });

  expect(result.kind).toBe("event");
  if (result.kind !== "event") {
    throw new Error("Expected mapped ReasonixCli event");
  }

  return result.event;
}

describe("ReasonixCli adapter", () => {
  it.each([
    ["start", "running"],
    ["tool_use", "using_tool"],
    ["finish", "completed"],
  ...
  it("handles malformed JSON in ingestReasonixCliHookJson", () => {
    const result = ingestReasonixCliHookJson("{invalid-json}");
    expect(result.kind).toBe("invalid");
  });
});
