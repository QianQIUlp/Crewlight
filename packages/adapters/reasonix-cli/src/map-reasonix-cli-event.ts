...
import {
  reasonixCliHookInputSchema,
  type ReasonixCliHookInput,
} from "./reasonix-cli-hook-input.js";

export type ReasonixCliAdapterResult =
...
export function mapReasonixCliEvent(input: unknown): ReasonixCliAdapterResult {
  const parsed = reasonixCliHookInputSchema.safeParse(input);
  if (!parsed.success) {
    return { kind: "invalid", reason: "Invalid ReasonixCli hook payload" };
  }
  ...
  if (!status) {
    return {
      kind: "ignored",
      reason: "Unsupported ReasonixCli hook event",
    };
  }
