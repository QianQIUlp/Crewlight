...
import { ingestPiAgentHookJson, mapPiAgentEvent } from "../src/index.js";

function eventFor(payload: Record<string, unknown>) {
  const result = mapPiAgentEvent({
    session_id: "pi-agent-session",
    cwd: "/tmp/pi-agent-project",
    ...payload,
  });

  expect(result.kind).toBe("event");
  if (result.kind !== "event") {
    throw new Error("Expected mapped PiAgent event");
  }

  return result.event;
}

describe("PiAgent adapter", () => {
  it.each([
    ["start", "running"],
    ["tool_use", "using_tool"],
    ["finish", "completed"],
  ...
  it("maps session identity, project path, and safe descriptive fields", () => {
    ...
    expect(normalized).toMatchObject({
      ...
      surface: "cli",
      status: "using_tool",
      sessionId: "pi-agent-session",
      projectPath: resolve("/tmp/pi-agent-project"),
      title: "tool_use",
  ...
  it("handles malformed JSON in ingestPiAgentHookJson", () => {
    const result = ingestPiAgentHookJson("{invalid-json}");
    expect(result.kind).toBe("invalid");
  });
});
