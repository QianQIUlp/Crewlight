...
import { ingestQoderHookJson, mapQoderEvent } from "../src/index.js";
...
function eventFor(payload: Record<string, unknown>) {
  ...
  if (result.kind !== "event") {
    throw new Error("Expected mapped Qoder event");
  }
...
describe("Qoder adapter", () => {
  ...
  it("maps session identity, project path, and safe descriptive fields", () => {
    ...
    expect(normalized).toMatchObject({
      ...
      projectPath: resolve("/tmp/qoder-project"),
      title: "PreToolUse",
  ...
  it("handles malformed JSON in ingestQoderHookJson", () => {
    const result = ingestQoderHookJson("{invalid-json}");
    expect(result.kind).toBe("invalid");
  });
});
