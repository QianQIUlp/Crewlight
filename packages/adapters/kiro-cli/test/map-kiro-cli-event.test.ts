...
import { ingestKiroCliHookJson, mapKiroCliEvent } from "../src/index.js";

function eventFor(payload: Record<string, unknown>) {
  const result = mapKiroCliEvent({
    session_id: "kiro-cli-session",
    cwd: "/tmp/kiro-cli-project",
    ...payload,
  });

  expect(result.kind).toBe("event");
  if (result.kind !== "event") {
    throw new Error("Expected mapped KiroCli event");
  }
...
describe("KiroCli adapter", () => {
  ...
  it("handles malformed JSON in ingestKiroCliHookJson", () => {
    const result = ingestKiroCliHookJson("{invalid-json}");
    expect(result.kind).toBe("invalid");
  });
});
